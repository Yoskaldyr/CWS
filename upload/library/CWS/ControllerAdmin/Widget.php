...
class CWS_ControllerAdmin_Widget extends XenForo_ControllerAdmin_Abstract
{
	...
	protected function _getWidgetAddEditResponse(array $widget)
	{
		...
		$callbackClass = isset($widget['callback_class']) ? $widget['callback_class'] : 'CWS_ControllerWidget_Abstract';
		$callbackClass = is_subclass_of($callbackClass, 'CWS_ControllerWidget_Abstract') ? $callbackClass : 'CWS_ControllerWidget_Abstract';

		$response->subView = call_user_func_array(array($callbackClass, 'renderOptions'), array($this, $widget));
		//die(Zend_Debug::dump($response->subView));
		return $response;
	}
	...
	protected function _getWidgetOrError($widgetId)
	{
		...
		$widgetClass = isset($widget['callback_class']) ? $widget['callback_class'] : 'CWS_ControllerWidget_Abstract';
		$widgetClass = is_subclass_of($widgetClass, 'CWS_ControllerWidget_Abstract') ? $widgetClass : 'CWS_ControllerWidget_Abstract';

		$widget = call_user_func_array(array($widgetClass, 'prepareWidget'), array($widget));
	...
	public function actionOptions()
	{
		...
		$widgetClass = is_subclass_of($class, 'CWS_ControllerWidget_Abstract') ? $class : 'CWS_ControllerWidget_Abstract';
		...
		return call_user_func_array(array($widgetClass, 'renderOptions'), array($this, $widget));
	}
	...
	public function actionSearchMethod()
	{
		...
		if(strpos($class, 'ControllerWidget') && XenForo_Application::autoload($class))
		{
	...
	public function actionSearchClass()
	{
		...
		if ($q !== '')
		{
			...
			foreach($filePaths as $filePath)
			{
				...
				if(strpos($class, $q) === 0 && strpos($class, 'ControllerWidget') && XenForo_Application::autoload($class))
				{
					$classes[] = $class;
				}
			}
